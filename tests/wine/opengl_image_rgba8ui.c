...
int main(void){static const char src[]="#version 430 core\nlayout(local_size_x=1) in;layout(rgba8ui,binding=0) uniform uimage2D img;void main(){imageStore(img,ivec2(0),uvec4(1u,2u,3u,4u));}";PIXELFORMATDESCRIPTOR d={0};HWND w;HDC dc;HGLRC c;int pf;GLuint s,pr,t;GLint ok;unsigned char value[4]={0,0,0,0};CS cs;SS ss;CO co;SI si;CP cp;AT at;LI li;PI pi;US us;GT gt;BT bt;TI ti;GI gi;GIT bind_image_textures;DC dispatch;GX get_image;w=CreateWindowA("STATIC","WineMetalGL image rgba8ui",WS_OVERLAPPEDWINDOW,0,0,64,64,0,0,0,0);if(!w)return 11;dc=GetDC(w);d.nSize=sizeof(d);d.nVersion=1;d.dwFlags=PFD_DRAW_TO_WINDOW|PFD_SUPPORT_OPENGL;d.iPixelType=PFD_TYPE_RGBA;d.cColorBits=32;pf=ChoosePixelFormat(dc,&d);if(!pf||!SetPixelFormat(dc,pf,&d))return 12;c=wglCreateContext(dc);if(!c||!wglMakeCurrent(dc,c))return 13;L(CS,cs,"glCreateShader");L(SS,ss,"glShaderSource");L(CO,co,"glCompileShader");L(SI,si,"glGetShaderiv");L(CP,cp,"glCreateProgram");L(AT,at,"glAttachShader");L(LI,li,"glLinkProgram");L(PI,pi,"glGetProgramiv");L(US,us,"glUseProgram");L(GT,gt,"glGenTextures");L(BT,bt,"glBindTexture");L(TI,ti,"glTexImage2D");L(GI,gi,"glBindImageTexture");L(GIT,bind_image_textures,"glBindImageTextures");L(DC,dispatch,"glDispatchCompute");L(GX,get_image,"glGetTexImage");s=cs(COMPUTE_SHADER);{const char*x=src;ss(s,1,&x,0);}co(s);si(s,COMPILE_STATUS,&ok);if(!ok)return 15;pr=cp();at(pr,s);li(pr);pi(pr,LINK_STATUS,&ok);if(!ok)return 16;us(pr);gt(1,&t);bt(TEXTURE_2D,t);ti(TEXTURE_2D,0,RGBA8UI,2,2,0,RGBA_INTEGER,UNSIGNED_BYTE,0);gi(0,t,0,0,0,WRITE_ONLY,RGBA8UI);GLuint image_list[1]={t};bind_image_textures(0,1,image_list);printf("WINEMETALGL_MULTI_BIND_IMAGES_OK\\n");dispatch(1,1,1);get_image(TEXTURE_2D,0,RGBA_INTEGER,UNSIGNED_BYTE,&value);printf("RGBA8UI value=%u,%u,%u,%u error=0x%x\n",value[0],value[1],value[2],value[3],(unsigned)glGetError());if(value[0]!=1||value[1]!=2||value[2]!=3||value[3]!=4)return 17;printf("WINEMETALGL_IMAGE_RGBA8UI_OK\n");wglMakeCurrent(0,0);wglDeleteContext(c);ReleaseDC(w,dc);DestroyWindow(w);return 0;}
