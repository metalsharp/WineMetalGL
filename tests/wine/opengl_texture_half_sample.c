...
int main(void){
    ...
    static const unsigned short data[]={0x3266,0x3666,0x38cd,0x3c00,0x3266,0x3666,0x38cd,0x3c00,0x3266,0x3666,0x38cd,0x3c00,0x3266,0x3666,0x38cd,0x3c00};
    ...
    w=CreateWindowA("STATIC","WineMetalGL half texture",WS_OVERLAPPEDWINDOW,0,0,64,64,0,0,0,0);if(!w)return 11;dc=GetDC(w);d.nSize=sizeof(d);d.nVersion=1;d.dwFlags=PFD_DRAW_TO_WINDOW|PFD_SUPPORT_OPENGL;d.iPixelType=PFD_TYPE_RGBA;d.cColorBits=32;pf=ChoosePixelFormat(dc,&d);if(!pf||!SetPixelFormat(dc,pf,&d))return 12;c=wglCreateContext(dc);if(!c||!wglMakeCurrent(dc,c))return 13;
    ...
    gt(1,&t);bt(TEXTURE_2D,t);tp(TEXTURE_2D,0x2801,NEAREST);tp(TEXTURE_2D,0x2800,NEAREST);ti(TEXTURE_2D,0,RGBA16F,2,2,0,RGBA,HALF_FLOAT,data);GLint loc=ul(pr,"tex");if(loc<0)return 18;u1(loc,0);glViewport(0,0,64,64);draw(TRIANGLES,0,3);glReadPixels(32,32,1,1,GL_RGBA,UNSIGNED_BYTE,pixel);printf("Half texture rgba=%u,%u,%u,%u error=0x%x\n",pixel[0],pixel[1],pixel[2],pixel[3],(unsigned)glGetError());if(pixel[0]<47||pixel[0]>55||pixel[1]<98||pixel[1]>106||pixel[2]<149||pixel[2]>157||pixel[3]<250)return 19;printf("WINEMETALGL_HALF_FLOAT_TEXTURE_SAMPLE_OK\n");wglMakeCurrent(0,0);wglDeleteContext(c);ReleaseDC(w,dc);DestroyWindow(w);return 0;
}
