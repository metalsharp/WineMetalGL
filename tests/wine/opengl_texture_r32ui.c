...
int main(void){
    ...
    static const char fs[]="#version 330 core\nuniform usampler2D tex;out vec4 color;void main(){uvec4 v=texture(tex,vec2(.5));color=vec4(float(v.r)/4294967295.0,0,0,1);}";
    static const unsigned int data[]={0x7f000000u,0x7f000000u,0x7f000000u,0x7f000000u};
    ...
    w=CreateWindowA("STATIC","WineMetalGL r32ui texture",WS_OVERLAPPEDWINDOW,0,0,64,64,0,0,0,0);if(!w)return 11;dc=GetDC(w);d.nSize=sizeof(d);d.nVersion=1;d.dwFlags=PFD_DRAW_TO_WINDOW|PFD_SUPPORT_OPENGL;d.iPixelType=PFD_TYPE_RGBA;d.cColorBits=32;pf=ChoosePixelFormat(dc,&d);if(!pf||!SetPixelFormat(dc,pf,&d))return 12;c=wglCreateContext(dc);if(!c||!wglMakeCurrent(dc,c))return 13;
    ...
    gt(1,&t);bt(TEXTURE_2D,t);tp(TEXTURE_2D,0x2801,NEAREST);tp(TEXTURE_2D,0x2800,NEAREST);ti(TEXTURE_2D,0,R32UI,2,2,0,RED_INTEGER,0x1405,data);GLint loc=ul(pr,"tex");if(loc<0)return 18;u1(loc,0);glViewport(0,0,64,64);draw(TRIANGLES,0,3);glReadPixels(32,32,1,1,GL_RGBA,UNSIGNED_BYTE,pixel);printf("Integer texture rgba=%u,%u,%u,%u error=0x%x\n",pixel[0],pixel[1],pixel[2],pixel[3],(unsigned)glGetError());if(pixel[0]<126||pixel[0]>128||pixel[1]!=0||pixel[2]!=0||pixel[3]<250)return 19;printf("WINEMETALGL_R32UI_TEXTURE_SAMPLE_OK\n");wglMakeCurrent(0,0);wglDeleteContext(c);ReleaseDC(w,dc);DestroyWindow(w);return 0;
}
