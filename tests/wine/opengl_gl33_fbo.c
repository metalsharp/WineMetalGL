...
int main(void){
 ...
 CreateShader cs;ShaderSource ss;CompileShader co;GetShaderiv si;CreateProgram cp;AttachShader at;LinkProgram li;GetProgramiv pi;UseProgram us;GenTextures gt;BindTexture bt;TexImage2D ti;GenFramebuffers gf;BindFramebuffer bf;FramebufferTexture2D ft;CheckFramebufferStatus fc;
 ...
 gt(1,&tex);bt(GL_TEXTURE_2D,tex);ti(GL_TEXTURE_2D,0,GL_RGBA8,2,2,0,GL_RGBA,GL_UNSIGNED_BYTE,0);gf(1,&fbo);bf(GL_FRAMEBUFFER,fbo);ft(GL_FRAMEBUFFER,GL_COLOR_ATTACHMENT0,GL_TEXTURE_2D,tex,0);status=fc(GL_FRAMEBUFFER);if(status!=GL_FRAMEBUFFER_COMPLETE){printf("FAIL FBO status=0x%x\n",status);return 18;}
