...
#include <stdio.h>
int main(void) {
    PIXELFORMATDESCRIPTOR p={0}; HWND w; HDC dc; HGLRC c; int f; unsigned char px[4]={0};
    ...
    f=ChoosePixelFormat(dc,&p); if(!f||!SetPixelFormat(dc,f,&p))return 12; c=wglCreateContext(dc); if(!c||!wglMakeCurrent(dc,c))return 13;
