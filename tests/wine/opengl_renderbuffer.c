...
int main(void){
 ...
 PIXELFORMATDESCRIPTOR d={0};HWND w;HDC dc;HGLRC c;int pf;GLuint v,f,pr,tex,fbo,rb;GLint ok;GLenum status;unsigned char px[4]={0};
 CreateShader cs;ShaderSource ss;CompileShader co;GetShaderiv si;CreateProgram cp;AttachShader at;LinkProgram li;GetProgramiv pi;UseProgram us;GenTextures gt;BindTexture bt;TexImage2D ti;GenFramebuffers gf;BindFramebuffer bf;FramebufferTexture2D ft;CheckFramebufferStatus fc;GenRenderbuffers gr;BindRenderbuffer br;RenderbufferStorage rs;FramebufferRenderbuffer fr;
 w=CreateWindowA("STATIC","WineMetalGL renderbuffer",WS_OVERLAPPEDWINDOW,0,0,64,64,0,0,0,0);if(!w)return 11;dc=GetDC(w);d.nSize=sizeof(d);d.nVersion=1;d.dwFlags=PFD_DRAW_TO_WINDOW|PFD_SUPPORT_OPENGL;d.iPixelType=PFD_TYPE_RGBA;d.cColorBits=32;pf=ChoosePixelFormat(dc,&d);if(!pf||!SetPixelFormat(dc,pf,&d))return 12;c=wglCreateContext(dc);if(!c||!wglMakeCurrent(dc,c))return 13;
 L(CreateShader,cs,"glCreateShader");L(ShaderSource,ss,"glShaderSource");L(CompileShader,co,"glCompileShader");L(GetShaderiv,si,"glGetShaderiv");L(CreateProgram,cp,"glCreateProgram");L(AttachShader,at,"glAttachShader");L(LinkProgram,li,"glLinkProgram");L(GetProgramiv,pi,"glGetProgramiv");L(UseProgram,us,"glUseProgram");L(GenTextures,gt,"glGenTextures");L(BindTexture,bt,"glBindTexture");L(TexImage2D,ti,"glTexImage2D");L(GenFramebuffers,gf,"glGenFramebuffers");L(BindFramebuffer,bf,"glBindFramebuffer");L(FramebufferTexture2D,ft,"glFramebufferTexture2D");L(CheckFramebufferStatus,fc,"glCheckFramebufferStatus");L(GenRenderbuffers,gr,"glGenRenderbuffers");L(BindRenderbuffer,br,"glBindRenderbuffer");L(RenderbufferStorage,rs,"glRenderbufferStorage");L(FramebufferRenderbuffer,fr,"glFramebufferRenderbuffer");
 ...
 gf(1,&fbo);bf(GL_FRAMEBUFFER,fbo);gr(1,&rb);br(GL_RENDERBUFFER,rb);rs(GL_RENDERBUFFER,GL_RGBA8,2,2);fr(GL_FRAMEBUFFER,GL_COLOR_ATTACHMENT0,GL_RENDERBUFFER,rb);status=fc(GL_FRAMEBUFFER);if(status!=GL_FRAMEBUFFER_COMPLETE){printf("FAIL FBO status=0x%x\n",status);return 18;}
 glViewport(0,0,2,2);glDrawArrays(GL_TRIANGLES,0,3);glReadPixels(1,1,1,1,GL_RGBA,GL_UNSIGNED_BYTE,px);printf("GL33 FBO readback rgba=%u,%u,%u,%u error=0x%x\n",px[0],px[1],px[2],px[3],(unsigned)glGetError());if(px[0]<47||px[0]>55||px[1]<98||px[1]>106||px[2]<149||px[2]>157||px[3]<250)return 19;printf("WINEMETALGL_RENDERBUFFER_OK\n");wglMakeCurrent(0,0);wglDeleteContext(c);ReleaseDC(w,dc);DestroyWindow(w);return 0;
}
