...
int main(void){
 ...
 CreateShader cs;GetTexLevelParameteriv get_level;ShaderSource ss;CompileShader co;GetShaderiv si;CreateProgram cp;AttachShader at;LinkProgram li;GetProgramiv pi;UseProgram us;GenTextures gt;BindTexture bt;TexImage2D ti;TexImage2DMultisample tm;GenFramebuffers gf;BindFramebuffer bf;FramebufferTexture2D ft;CheckFramebufferStatus fc;BlitFramebuffer blit;
 ...
 gt(1,&tex);bt(GL_TEXTURE_2D_MULTISAMPLE,tex);tm(GL_TEXTURE_2D_MULTISAMPLE,4,GL_RGBA8,2,2,1);GLint samples=0;get_level(GL_TEXTURE_2D_MULTISAMPLE,0,0x9106,&samples);if(samples<2)return 18;printf("WINEMETALGL_MULTISAMPLE_STORAGE_OK\\n");gf(1,&fbo);bf(GL_FRAMEBUFFER,fbo);ft(GL_FRAMEBUFFER,GL_COLOR_ATTACHMENT0,GL_TEXTURE_2D_MULTISAMPLE,tex,0);status=fc(GL_FRAMEBUFFER);if(status!=GL_FRAMEBUFFER_COMPLETE){printf("FAIL FBO status=0x%x\n",status);return 18;}
